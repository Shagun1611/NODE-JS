...
const server=http.createServer((req,res)=>{
    ...
    req.on('end',()=>{               //because it is taking more time  to get the data so ending point is taking less time than starting point so the starting point is printing after the ending point
...
// nodejs is a single thread ed event driven non blocking language 
//unit of thread is called process
//when we want to increase performance we have to increase number of thread
//node js assign single thread for every process
//the event loop in nodejs is a single thread ,non blocking i/mechanism that allow nodejs  to perform multiple task at once without blocking the main thread. this can be achieved by offloading to the workerpool
